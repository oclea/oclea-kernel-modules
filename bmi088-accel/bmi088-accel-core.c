...
/* Scale table from the 6.1 driver */
static const int bmi088_scale_table[][2] = {{0, 897}, {0, 1794}, {0, 3589}, {0, 7178}};
				 ...
				 int *val, int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_SCALE:
		...
		case IIO_ACCEL:
			...
			// Driver changes:
			// Original driver got the value mostly correct, but out
			// by a power of 10
			// Use a later driver's LUT implementation

			// get the two bits that represent which scaling mode to use
			ret = *val & 0x3;

			*val = bmi088_scale_table[ret][0];
			*val2 = bmi088_scale_table[ret][1];
	...
	int irq, const char *name, bool block_supported)
{
	...
	// Override the default iio name (iio:devicen)
	dev_set_name(&indio_dev->dev, "iio:bmi088-accel");
