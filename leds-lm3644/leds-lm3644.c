...
#define LM3644_MAX_DEVICES 3

static DECLARE_WAIT_QUEUE_HEAD(lm3644_poll_wait_queue);

enum lm3644_mode { MODES_STANDBY = 0, MODES_IR, MODES_TORCH, MODES_FLASH };
...
static int lm3644_major;
static DEFINE_IDA(lm3644_minor_ida);
static struct class *lm3644_class = NULL;
...
static int lm3644_disable_tx(struct lm3644_chip_data *chip)
{
	unsigned int val;
	int ret;
...
/* read flag */
static int lm3644_read_flag(struct lm3644_chip_data *chip)
{
	unsigned int flag1, flag2;
	int ret;

	ret = regmap_read(chip->regmap, REG_FLAG1, &flag1);
	if (ret < 0) {
		dev_err(chip->dev, "Failed to read REG_FLAG1 register\n");
		return ret;
	}

	ret = regmap_read(chip->regmap, REG_FLAG2, &flag2);
	if (ret < 0) {
		dev_err(chip->dev, "Failed to read REG_FLAG2 register\n");
		return ret;
	}

	dev_info(chip->dev, "[flag1] 0x%x, [flag2] 0x%x\n", flag1, flag2 & 0x1f);
	return ret;
}

/* chip control */
static int lm3644_control(struct lm3644_chip_data *chip, u8 brightness,
			  enum lm3644_mode opmode)
{
	int ret;
	int val;
	...
	dev_info(chip->dev, "[%s]: brightness = %u, opmode = %d\n", __func__,
		brightness, opmode);
	...
	if (opmode != MODES_IR) {
		if (chip->pdata->use_simulative_pwm && chip->pwm != NULL) {
			...
			dev_info(chip->dev, "Simulative PWM disabled\n");
		}
	}
				       ...
				       enum led_brightness brightness)
{
	...
	if (ret < 0)
		dev_err(chip->dev, "Set torch brightness failed %d\n", ret);
	else
...
/* ir mode */
static int lm3644_ir_flash_timeout_set(struct lm3644_chip_data *chip)
{
	unsigned int val;
	int ret;

	// Constrain to acceptable flash timeout values
	if (chip->pdata->flash_timeout > 0xf) {
		val = 0xf;
	} else {
		val = chip->pdata->flash_timeout;
	}

	ret = regmap_write(chip->regmap, REG_TIMING_CONF, val);
	if (ret < 0) {
		dev_err(chip->dev,
			"Failed to write REG_TIMING_CONF register\n");
	} else {
		dev_info(chip->dev, "Flash timeout configuration is %i\n", val);
	}
				    ...
				    enum led_brightness brightness)
{
	...
	if (ret < 0)
		dev_err(chip->dev, "Set IR Brightness Failed %d\n", ret);
	else
		lm3644_read_flag(chip);
...
static ssize_t lm3644_ir_write(struct file *filp, const char *buf, size_t len,
			       loff_t *fseek)
{
	int ret = 0;
...
static ssize_t lm3644_ir_read(struct file *filp, char *buf, size_t len,
			      loff_t *fseek)
{
	int ret = 0;
	int data = 0;
...
static unsigned int lm3644_poll(struct file *filp, poll_table *wait)
{
	...
	if (atomic_read(&chip->ito_exception)) {
		...
		if (ret < 0)
			dev_err(chip->dev, "Set STANDBY mode failed %d\n", ret);
		else
			lm3644_read_flag(chip);
...
static unsigned int lm3644_ir_convert_current_to_brightness(int flash_current)
{
	return (flash_current - 10900) / 11725;
}

static int lm3644_ir_convert_brightness_to_current(unsigned int brightness)
{
	return (brightness * 11725 + 10900);
}
...
static int lm3644_ir_set_data(struct lm3644_chip_data *chip, lm3644_data params)
{
	...
	if (SET_BRIGHTNESS_EVENT == params.event) {
		brightness =
			lm3644_ir_convert_current_to_brightness(params.data);
		if (brightness > 0x7F) {
			dev_err(chip->dev,
				"brightness %d is higher then the max value 0x7F, set to 0x7F\n",
				brightness);
			brightness = 0x7F;
		}
...
static int lm3644_ir_get_data(struct lm3644_chip_data *chip, lm3644_data params)
{
	...
	if (GET_BRIGHTNESS_EVENT == params.event)
		data = lm3644_ir_convert_brightness_to_current(
			chip->pdata->brightness);
...
static long lm3644_ir_ioctl(struct file *filp, unsigned int cmd,
			    unsigned long arg)
{
	int ret = 0;
	struct lm3644_chip_data *chip = filp->private_data;
...
#ifdef CONFIG_COMPAT
static long lm3644_ir_compat_ioctl(struct file *filp, unsigned int cmd,
				   unsigned long arg)
{
				    ...
				    const char *buff, size_t count)
{
	...
	dev_err(chip->dev,
		"lm3644_reg_opt_store, reg:0x%x, val_after_set:0x%x.\n", addr,
		val);
...
static ssize_t lm3644_id_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct lm3644_chip_data *chip =
...
static DEVICE_ATTR(pwm_period, S_IRUGO | S_IWUSR, lm3644_pwm_period_show,
		   lm3644_pwm_period_store);
static DEVICE_ATTR(pwm_duty, S_IRUGO | S_IWUSR, lm3644_pwm_duty_show,
		   lm3644_pwm_duty_store);
static DEVICE_ATTR(id, S_IRUGO, lm3644_id_show, NULL);
static DEVICE_ATTR(reg_opt, S_IRUGO | S_IWUSR, lm3644_reg_opt_show,
		   lm3644_reg_opt_store);

static struct attribute *lm3644_ir_attrs[] = { &dev_attr_pwm_period.attr,
...
static struct lm3644_platform_data *lm3644_parse_dt(struct i2c_client *client)
{
	...
	pdata->ito_detect_gpio =
		of_get_named_gpio(np, "lm3644,ito-detect-gpio", 0);

	ret = of_property_read_u32(np, "lm3644,flash-timeout",
				   &pdata->flash_timeout);
	if (ret < 0) {
		dev_err(&client->dev,
			"No flash timeout specified, use default value\n");
		pdata->flash_timeout = -1;
	}

	pdata->pass_mode = of_property_read_bool(np, "lm3644,pass-mode");
	pdata->disable_tx = of_property_read_bool(np, "lm3644,disable-tx");

	pdata->use_simulative_pwm =
		of_property_read_bool(np, "lm3644,use-simulative-pwm");

	if (pdata->use_simulative_pwm) {
		ret = of_property_read_u32(np, "lm3644,period-us",
					   &pdata->pwm_period_us);
		if (ret < 0) {
			dev_err(&client->dev,
		...
		ret = of_property_read_u32(np, "lm3644,duty-us",
					   &pdata->pwm_duty_us);
		if (ret < 0) {
			dev_err(&client->dev,
...
static int lm3644_initialize_cdev(struct i2c_client *client,
				  struct lm3644_chip_data *chip)
{
	int rc = 0;
	cdev_init(&chip->cdev, &lm3644_ir_fops);
	chip->cdev.owner = THIS_MODULE;
	chip->minor = ida_simple_get(&lm3644_minor_ida, 0, LM3644_MAX_DEVICES,
				     GFP_KERNEL);
	if (chip->minor < 0) {
		rc = -EINVAL;
		dev_err(&client->dev,
			"lm3644 Unable to obtain minor for device\n");
		goto ida_failed;
	}

	chip->dev_num = MKDEV(lm3644_major, chip->minor);

	chip->chr_dev = device_create(lm3644_class, NULL, chip->dev_num, chip,
				      client->dev.of_node->name);

	rc = cdev_add(&chip->cdev, chip->dev_num, 1);
	if (rc < 0) {
		dev_err(&client->dev,
			"lm3644 Unable to add character device\n");
		goto cdev_add_error;
	}
...
cdev_add_error:
	ida_simple_remove(&lm3644_minor_ida, chip->minor);
	if (chip->chr_dev)
		device_destroy(lm3644_class, chip->dev_num);
ida_failed:
	return rc;
}
			...
			const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct lm3644_chip_data),
			    GFP_KERNEL);
	if (!chip)
		return -ENOMEM;
	...
	if (pdata->disable_tx) {
		err = lm3644_disable_tx(chip);
		if (err < 0)
			goto err_free_ito_irq;
	}

	if (pdata->flash_timeout >= 0) {
		err = lm3644_ir_flash_timeout_set(chip);
		if (err < 0)
			goto err_free_ito_irq;
	}

	err = lm3644_initialize_cdev(client, chip);
	if (err < 0) {
		dev_err(&client->dev, "Failed to initialize cdev\n");
		goto err_free_ito_irq;
	}

	/* torch mode */
	char torch_name[30];
	snprintf(torch_name, 30, "%s_ir_torch", client->dev.of_node->name);
	chip->cdev_torch.name = torch_name;
	...
	/* ir mode */
	char ir_name[30];
	snprintf(ir_name, 30, "%s_ir", client->dev.of_node->name);
	chip->cdev_ir.name = ir_name;
...
err_free_torch_gpio:
	if (gpio_is_valid(pdata->torch_gpio)) {
		gpio_set_value_cansleep(pdata->torch_gpio, 0);
		gpio_free(pdata->torch_gpio);
	}
err_free_tx_gpio:
	if (gpio_is_valid(pdata->tx_gpio)) {
		gpio_set_value_cansleep(pdata->tx_gpio, 0);
		gpio_free(pdata->tx_gpio);
	}
err_free_hwen_gpio:
	if (gpio_is_valid(pdata->hwen_gpio)) {
		/* Pull HWEN to ground to reduce power */
		gpio_set_value_cansleep(pdata->hwen_gpio, 0);
...
static int lm3644_remove(struct i2c_client *client)
{
	...
	if (gpio_is_valid(chip->pdata->hwen_gpio)) {
		gpio_set_value_cansleep(chip->pdata->hwen_gpio, 0);
		gpio_free(chip->pdata->hwen_gpio);
	}

	if (gpio_is_valid(chip->pdata->tx_gpio)) {
		gpio_set_value_cansleep(chip->pdata->tx_gpio, 0);
		gpio_free(chip->pdata->tx_gpio);
	}

	if (gpio_is_valid(chip->pdata->torch_gpio)) {
		gpio_set_value_cansleep(chip->pdata->torch_gpio, 0);
...
static const struct i2c_device_id lm3644_id[] = { { LM3644_NAME, 0 }, {} };

MODULE_DEVICE_TABLE(i2c, lm3644_id);

static const struct of_device_id of_lm3644_leds_match[] = {
	{
		.compatible = "ti,lm3644",
	},
	{},
};
MODULE_DEVICE_TABLE(of, of_lm3644_leds_match);

static struct i2c_driver lm3644_i2c_driver = {
	.driver = {
		.name = LM3644_NAME,
		.of_match_table = of_lm3644_leds_match,
	},
	.probe = lm3644_probe,
...
static int __init lm3644_driver_init(void)
{
	int rc;
	dev_t lm3644_first_major;

	rc = alloc_chrdev_region(&lm3644_first_major, 0, LM3644_MAX_DEVICES,
				 LM3644_CLASS_NAME);
	if (rc < 0) {
		goto alloc_chrdev_region_failed;
	}

	lm3644_major = MAJOR(lm3644_first_major);
	lm3644_class = class_create(THIS_MODULE, LM3644_CLASS_NAME);
	if (lm3644_class == NULL) {
		rc = PTR_ERR(lm3644_class);
		pr_err("class_create failed: %d\n", rc);
		goto class_create_failed;
	}

	rc = i2c_add_driver(&lm3644_i2c_driver);
	if (rc) {
...
add_driver_failed:
	class_destroy(lm3644_class);
class_create_failed:
	unregister_chrdev_region(MKDEV(lm3644_major, 0), LM3644_MAX_DEVICES);
alloc_chrdev_region_failed:
	return rc;
}

static void __init lm3644_driver_exit(void)
{
...
module_init(lm3644_driver_init);
module_exit(lm3644_driver_exit);
