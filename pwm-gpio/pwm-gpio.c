...
static DEFINE_SPINLOCK(gpio_pwm_lock);
...
static enum hrtimer_restart gpio_pwm_timer(struct hrtimer *timer)
{
    ...
    unsigned long flags;
    spin_lock_irqsave(&gpio_pwm_lock, flags);

#if 0
    if (!pwm_is_enabled(pc->chip.pwms)) {
        gpio_pwm_off(pc);
        pc->pin_on = false;
        spin_unlock_irqrestore(&gpio_pwm_lock, flags);
                ...
                int duty_ns, int period_ns)
{
    ...
    int need_start_timer = 0;
    unsigned long flags;

    spin_lock_irqsave(&gpio_pwm_lock, flags);

    // check if the timer should have previously been running
    if (!pc->off_time || !pc->on_time) {
        // need start timer because it was not started
        need_start_timer = 1;
    }
